...
# compute cost
def costfunc(tetas, x_norm, y):
    n_samples, n_features = x_norm.shape
    z = tetas.dot(x_norm.transpose())
    h = linear(z)
    e = np.subtract(h, y.T)
    se = np.square(e)
    return np.multiply(1/(2*n_samples), se.sum())


#compute gradient
def gradfunc(tetas, x_norm, y):
    n_samples, n_features = x_norm.shape
    z = tetas.dot(x_norm.transpose())
    h = linear(z)
    e = np.subtract(h, y.T)
    # grad = np.zeros_like(tetas)
    grad = np.multiply((1/n_samples), np.dot(e, x_norm))
    return grad



#learn tetas
def grad_descent(tetas, x_norm, y, iterations = 9900, alpha = 0.01):
